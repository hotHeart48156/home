pub struct SerialParser {
    pub serials: Vec<proc_macro2::TokenStream>,
}
...
impl syn::parse::Parse for SerialParser {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        let all_block: ParseBuffer;
        let mut block: ParseBuffer;
        let mut serials: Vec<proc_macro2::TokenStream> = vec![];
        ...
        return Ok(SerialParser { serials });
    }
}
